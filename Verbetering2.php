...
if (isset($_SESSION['question_id'])) {
    $stmt = $db->prepare("SELECT question, option_a, option_b, next_question_a, next_question_b FROM questions WHERE id = 0");
    $stmt->execute();
} else {
    $stmt = $db->prepare("SELECT question, option_a, option_b, next_question_a, next_question_b FROM questions WHERE id = 1");
    $stmt->execute();
}
...
if (isset($_POST['update_question'])) {
    $stmt = $db->prepare("UPDATE questions SET question = ?, option_a = ?, option_b = ?, next_question_a = ?, next_question_b = ? WHERE id = ?");
